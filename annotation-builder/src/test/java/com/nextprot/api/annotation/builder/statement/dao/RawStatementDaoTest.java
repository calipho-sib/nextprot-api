...
public class RawStatementDaoTest extends AnnotationBuilderBaseTest {
	...
	public void findAllNormalStatements() {
		...
		statements.stream().forEach(s -> System.out.println(i.getAndIncrement() + " - " + s.getValue(StatementField.ANNOT_ISO_ID)));
		List<RawStatement> statement = statements.stream().filter(s -> s.getValue(StatementField.ANNOT_ISO_ID).equals("c075d4a6b44e95faec7d8b109166744b")).collect(Collectors.toList());
	}
