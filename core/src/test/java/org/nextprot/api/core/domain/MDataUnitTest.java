...
public class MDataUnitTest {
    ...
    public void testMdata() {
        ...
        Mdata.MDataContext mc = Mdata.convertXmlToMDataContext(xml);
        assertEquals(mc.getMetadata().getCellLine().getContent(), "Jurkat E6.1 [CVCL_0367].");
        assertEquals(mc.getMetadata().getCellLine().getIsVariable(), "N");
        assertEquals(mc.getMetadata().getDataAnalysisProcedure().getLabel(), "Data analysis procedure");
