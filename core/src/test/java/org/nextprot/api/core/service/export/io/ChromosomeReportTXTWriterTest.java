...
public class ChromosomeReportTXTWriterTest {
    ...
    public void writeChromosomeReport() throws Exception {
        ...
        Assert.assertEquals("Gene          neXtProt      Chromosomal  Start      Stop       Protein          Prote- Anti- 3D    Dise- Iso-  Vari-  PTMs Description", observedLines[16]);
        Assert.assertEquals("name          AC            position     position   position   existence        omics  body        ase   forms ants        ", observedLines[17]);
        Assert.assertEquals("SRY           NX_Q05066     Yp11.2          2786855    2787699 protein level    no     yes   yes   yes       1    47     1 Sex-determining region Y protein", observedLines[19]);
        Assert.assertEquals("RBMY1C        NX_P0DJD4     Yq11.23               -          - protein level    no     yes   no    no        1     0     0 RNA-binding motif protein, Y chromosome, family 1 member C", observedLines[20]);
        Assert.assertEquals("MFSD14A       NX_Q96MC6     1p21.2        100038097  100083377 protein level    no     no    no    no        1   141     3 Hippocampus abundant transcript 1 protein", observedLines[21]);
    }
}