...
from nextprot.sequence_identifiers antibody
  inner join nextprot.identifier_resource_assoc ira on (antibody.identifier_id=ira.identifier_id)
  ...
  inner join nextprot.cv_datasources ds on (ira.datasource_id=ds.cv_id and ds.cv_name != 'HPA')
  inner join nextprot.cv_database_categories dbc on (dbc.cv_id=db.cv_category_id)
where xr.resource_id in (:ids)
